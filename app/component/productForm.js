  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      {propertiesToFill.length > 0 && (
        <div>
          {propertiesToFill.map((property) => (
            <div key={property.name} className="gap-2 items-center">
              <label>
      ...
      <div className="mb-2 flex flex-wrap gap-2">
        ...
        >
          {images?.length &&
            images.map((link) => (
              <div
                key={link}
                className="shadow-sm rounded-lg p-2 flex items-center justify-center w-24 h-24 overflow-y-hidden "
              >
        ...
        {isUploading && <Spinner />}
        <label
          className="w-24 h-24 cursor-pointer text-center items-center 
