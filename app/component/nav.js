...
export default function Navigator({ show }) {

  const inactiveLink = " flex gap-2 text-gray-700 p-2 ";
  const activeLink = inactiveLink + " bg-purple-300 rounded-lg ";
  ...
  return (
    ...
    "top-0 text-gray-500 p-4 fixed w-full bg-bgGray h-screen md:static md:w-auto flex flex-col"}>
      ...
      <button onClick={logout} className="bg-white p-2 px-4 rounded-lg text-black text-lg mt-auto">Logout</button>
    </aside>
  );
}